    ...
    bool findSolution(unsigned char nonce[32])
    {
        ...
        for (unsigned int tick = 0; tick < MAX_INPUT_DURATION; tick++)
        {
            ...
            for (unsigned int inputNeuronIndex = 0; inputNeuronIndex < NUMBER_OF_INPUT_NEURONS + INFO_LENGTH; inputNeuronIndex++)
            {
                ...
                *((long long*)counters) = 0;
                size_t idx;
                offset = inputNeuronIndex * ((INFO_LENGTH + NUMBER_OF_INPUT_NEURONS + DATA_LENGTH) / 32);
                for (unsigned int anotherInputNeuronIndex = 0; anotherInputNeuronIndex < (INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH) / 32; anotherInputNeuronIndex++)
                {
                    const unsigned int op = anotherInputNeuronIndex >> 1;
                    for (unsigned int another_offset = 0; another_offset < 32; another_offset++)
                    {
                        const unsigned long long synapse = synapses.input[offset] >> ((another_offset) << 1);

                        idx = (((neurons.input[op] >> (another_offset + (anotherInputNeuronIndex & 1) * 32))) ^ (synapse >> 1)) & 1;
                        counters[idx] += (uint32_t)(synapse & 1);
                    }
                    offset++;
                }
        ...
        for (unsigned int tick = 0; tick < MAX_OUTPUT_DURATION; tick++)
        {
            ...
            for (unsigned int outputNeuronIndex = 0; outputNeuronIndex < NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH; outputNeuronIndex++)
            {
                ...
                *((long long*)counters) = 0;
                size_t idx;
                offset = outputNeuronIndex * ((INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH) / 32);
                for (unsigned int anotherOutputNeuronIndex = 0; anotherOutputNeuronIndex < (INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS + DATA_LENGTH) / 32; anotherOutputNeuronIndex++)
                {
                    const unsigned int op = anotherOutputNeuronIndex >> 1;
                    for (unsigned int another_offset = 0; another_offset < 32; another_offset++)
                    {
                        const unsigned long long synapse = synapses.output[offset] >> ((another_offset) << 1);

                        idx = (((neurons.output[op] >> (another_offset + (anotherOutputNeuronIndex & 1) * 32))) ^ (synapse >> 1)) & 1;
                        counters[idx] += (uint32_t)(synapse & 1);
                    }
                    offset++;
                }
        ...
        {
            score += (64 - __builtin_popcountll(data[i] ^ neurons.output[(INFO_LENGTH + NUMBER_OF_OUTPUT_NEURONS) / 64 + i]));
        }
